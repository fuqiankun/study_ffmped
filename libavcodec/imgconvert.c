...
int avpicture_fill(AVPicture *picture, uint8_t *ptr, int pix_fmt, int width,
	...
	if (avcodec_check_dimensions(NULL, width, height)) goto fail;
	...
	switch (pix_fmt) {
		...
		case PIX_FMT_PAL8:
			...
			picture->data[0] = ptr;
			picture->data[1] =
					ptr + size2;	// palette is stored here as 256 32 bit words
			picture->data[2] = NULL;
			picture->linesize[0] = width;
...
void avpicture_free(AVPicture *picture) { av_free(picture->data[0]); }
	...
	switch (pf->pixel_type) {
		...
		case FF_PIXEL_PLANAR:
			if (pf->x_chroma_shift == 0 && pf->y_chroma_shift == 0) {
				...
				bits = pf->depth +
							 ((2 * pf->depth) >> (pf->x_chroma_shift + pf->y_chroma_shift));
			}
			break;
...
void ff_img_copy_plane(uint8_t *dst, int dst_wrap, const uint8_t *src,
...
void img_copy(AVPicture *dst, const AVPicture *src, int pix_fmt, int width,
	...
	switch (pf->pixel_type) {
		case FF_PIXEL_PACKED:
			...
			ff_img_copy_plane(dst->data[0], dst->linesize[0], src->data[0],
												src->linesize[0], bwidth, height);
			break;
		case FF_PIXEL_PLANAR:
			for (i = 0; i < pf->nb_channels; i++) {
				...
				ff_img_copy_plane(dst->data[i], dst->linesize[i], src->data[i],
													src->linesize[i], bwidth, h);
			}
			break;
		case FF_PIXEL_PALETTE:
			ff_img_copy_plane(dst->data[0], dst->linesize[0], src->data[0],
												src->linesize[0], width, height);
			// copy the palette
			ff_img_copy_plane(dst->data[1], dst->linesize[1], src->data[1],
												src->linesize[1], 4, 256);
			break;
	}
}

static void yuv422_to_yuv420p(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	const uint8_t *p, *p1;
...
static void uyvy422_to_yuv420p(AVPicture *dst, const AVPicture *src, int width,
															 int height) {
	const uint8_t *p, *p1;
	uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
	int w;
...
static void uyvy422_to_yuv422p(AVPicture *dst, const AVPicture *src, int width,
															 int height) {
	const uint8_t *p, *p1;
	uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
	int w;
...
static void yuv422_to_yuv422p(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	const uint8_t *p, *p1;
	uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
	int w;
...
static void yuv422p_to_yuv422(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	uint8_t *p, *p1;
...
static void yuv422p_to_uyvy422(AVPicture *dst, const AVPicture *src, int width,
															 int height) {
	uint8_t *p, *p1;
	const uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
	int w;
...
static void uyvy411_to_yuv411p(AVPicture *dst, const AVPicture *src, int width,
															 int height) {
	const uint8_t *p, *p1;
	uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
	int w;
...
static void yuv420p_to_yuv422(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	int w, h;
...
static void yuv420p_to_uyvy422(AVPicture *dst, const AVPicture *src, int width,
															 int height) {
	int w, h;
	uint8_t *line1, *line2, *linesrc = dst->data[0];
	uint8_t *lum1, *lum2, *lumsrc = src->data[0];
...
#define Y_CCIR_TO_JPEG(y)                                               \
	cm[((y)*FIX(255.0 / 219.0) + (ONE_HALF - 16 * FIX(255.0 / 219.0))) >> \
		 SCALEBITS]
...
#define C_CCIR_TO_JPEG(y)                                                  \
	cm[(((y)-128) * FIX(127.0 / 112.0) + (ONE_HALF + (128 << SCALEBITS))) >> \
		 SCALEBITS]
...
#define RGB_TO_U(r1, g1, b1, shift)                               \
	(((-FIX(0.16874) * r1 - FIX(0.33126) * g1 + FIX(0.50000) * b1 + \
		 (ONE_HALF << shift) - 1) >>                                  \
		(SCALEBITS + shift)) +                                        \
	 128)

#define RGB_TO_V(r1, g1, b1, shift)                              \
	(((FIX(0.50000) * r1 - FIX(0.41869) * g1 - FIX(0.08131) * b1 + \
		 (ONE_HALF << shift) - 1) >>                                 \
		(SCALEBITS + shift)) +                                       \
	 128)
...
static void img_apply_table(uint8_t *dst, int dst_wrap, const uint8_t *src,
														int src_wrap, int width, int height,
...
/* 4x1 -> 1x1 */
static void shrink41(uint8_t *dst, int dst_wrap, const uint8_t *src,
										 int src_wrap, int width, int height) {
	int w;
	const uint8_t *s;
	uint8_t *d;
...
/* 2x1 -> 1x1 */
static void shrink21(uint8_t *dst, int dst_wrap, const uint8_t *src,
										 int src_wrap, int width, int height) {
	int w;
	const uint8_t *s;
	uint8_t *d;
...
/* 1x2 -> 1x1 */
static void shrink12(uint8_t *dst, int dst_wrap, const uint8_t *src,
										 int src_wrap, int width, int height) {
	int w;
	uint8_t *d;
	const uint8_t *s1, *s2;
...
/* 2x2 -> 1x1 */
void ff_shrink22(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
...
/* 4x4 -> 1x1 */
void ff_shrink44(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
								 int width, int height) {
	int w;
		...
		for (w = width; w > 0; w--) {
			d[0] =
					(s1[0] + s1[1] + s1[2] + s1[3] + s2[0] + s2[1] + s2[2] + s2[3] +
					 s3[0] + s3[1] + s3[2] + s3[3] + s4[0] + s4[1] + s4[2] + s4[3] + 8) >>
					4;
			s1 += 4;
			s2 += 4;
			s3 += 4;
...
/* 1x1 -> 2x1 */
static void grow21(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
									 int width, int height) {
	for (; height > 0; height--) {
...
/* 1x1 -> 2x2 */
static void grow22(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
									 int width, int height) {
	for (; height > 0; height--) {
		grow21_line(dst, src, width);
		if (height % 2) src += src_wrap;
		dst += dst_wrap;
	}
}

/* 1x1 -> 4x1 */
static void grow41(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
									 int width, int height) {
	for (; height > 0; height--) {
...
/* 1x1 -> 4x4 */
static void grow44(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
									 int width, int height) {
	for (; height > 0; height--) {
		grow41_line(dst, src, width);
		if ((height & 3) == 1) src += src_wrap;
		dst += dst_wrap;
	}
}

/* 1x2 -> 2x1 */
static void conv411(uint8_t *dst, int dst_wrap, const uint8_t *src,
										int src_wrap, int width, int height) {
	int w, c;
	const uint8_t *s1, *s2;
	uint8_t *d;
...
static void build_rgb_palette(uint8_t *palette, int has_alpha) {
	...
	for (r = 0; r < 6; r++) {
		for (g = 0; g < 6; g++) {
			for (b = 0; b < 6; b++) {
				pal[i++] = (0xff << 24) | (pal_value[r] << 16) | (pal_value[g] << 8) |
									 pal_value[b];
			}
		}
	}
	if (has_alpha) pal[i++] = 0;
	while (i < 256) pal[i++] = 0xff000000;
}
...
static void mono_to_gray(AVPicture *dst, const AVPicture *src, int width,
												 int height, int xor_mask) {
...
static void monowhite_to_gray(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	mono_to_gray(dst, src, width, height, 0xff);
}

static void monoblack_to_gray(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	mono_to_gray(dst, src, width, height, 0x00);
}

static void gray_to_mono(AVPicture *dst, const AVPicture *src, int width,
												 int height, int xor_mask) {
	int n;
	const uint8_t *s;
	uint8_t *d;
...
static void gray_to_monowhite(AVPicture *dst, const AVPicture *src, int width,
															int height) {
	gray_to_mono(dst, src, width, height, 0xff);
}

static void gray_to_monoblack(AVPicture *dst, const AVPicture *src, int width,
															int height) {
...
to convert from any format to any format, the following constraints must be
satisfied:
...
int img_convert(AVPicture *dst, int dst_pix_fmt, const AVPicture *src,
	...
	if (src_pix_fmt < 0 || src_pix_fmt >= PIX_FMT_NB || dst_pix_fmt < 0 ||
			dst_pix_fmt >= PIX_FMT_NB)
		return -1;
	...
	if (ce->convert) {
		ce->convert(dst, src, dst_width,
								dst_height);	// specific conversion routine
		return 0;
	}
	...
	if (is_yuv_planar(dst_pix) && src_pix_fmt == PIX_FMT_GRAY8)	// gray to YUV
	{
		...
		if (dst_pix->color_type == FF_COLOR_YUV_JPEG) {
			ff_img_copy_plane(dst->data[0], dst->linesize[0], src->data[0],
												src->linesize[0], dst_width, dst_height);
		} else {
			img_apply_table(dst->data[0], dst->linesize[0], src->data[0],
											src->linesize[0], dst_width, dst_height, y_jpeg_to_ccir);
		}
	...
	if (is_yuv_planar(src_pix) && dst_pix_fmt == PIX_FMT_GRAY8)	// YUV to gray
	{
		if (src_pix->color_type == FF_COLOR_YUV_JPEG) {
			...
			img_apply_table(dst->data[0], dst->linesize[0], src->data[0],
											src->linesize[0], dst_width, dst_height, y_ccir_to_jpeg);
		}
		return 0;
	}
	...
	if (is_yuv_planar(dst_pix) && is_yuv_planar(src_pix))	// YUV to YUV planar
	{
		...
		void (*resize_func)(uint8_t * dst, int dst_wrap, const uint8_t *src,
												int src_wrap, int width, int height);
		...
		for (i = 1; i <= 2; i++)
			resize_func(dst->data[i], dst->linesize[i], src->data[i],
									src->linesize[i], dst_width >> dst_pix->x_chroma_shift,
									dst_height >> dst_pix->y_chroma_shift);

		// if yuv color space conversion is needed, we do it here on the destination
		// image
		if (dst_pix->color_type != src_pix->color_type) {
			...
			img_apply_table(dst->data[0], dst->linesize[0], dst->data[0],
											dst->linesize[0], dst_width, dst_height, y_table);

			for (i = 1; i <= 2; i++)
				img_apply_table(dst->data[i], dst->linesize[i], dst->data[i],
												dst->linesize[i], dst_width >> dst_pix->x_chroma_shift,
...
no_chroma_filter:	// try to use an intermediate format
	...
	if (src_pix_fmt == PIX_FMT_YUV422 || dst_pix_fmt == PIX_FMT_YUV422) {
		...
		if (src_pix->color_type ==
				FF_COLOR_YUV_JPEG)	// yuv444 is the normalized format
	...
	if (avpicture_alloc(tmp, int_pix_fmt, dst_width, dst_height) < 0) return -1;

	ret = -1;

	if (img_convert(tmp, int_pix_fmt, src, src_pix_fmt, src_width, src_height) <
			0)
		goto fail1;

	if (img_convert(dst, dst_pix_fmt, tmp, int_pix_fmt, dst_width, dst_height) <
			0)
		goto fail1;
	ret = 0;
