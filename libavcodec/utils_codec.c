...
// 计算各种图像格式要求的图像长宽的字节对齐数，是1 个还是2 个，4 个，8 个，16
// 个字节对齐。
void avcodec_align_dimensions(AVCodecContext *s, int *width, int *height) {
	...
	switch (s->pix_fmt) {
		...
		case PIX_FMT_YUVJ444P:	// FIXME check for non mpeg style codecs and use
														// less alignment
			w_align = 16;
			h_align = 16;
			break;
...
// 每次取internal_buffer_count
// 数据项，用base[0]来判断是否已分配内存，用data[0]来判断是否已被占用。base[]和data[]有多重意义。
// 在avcodec_alloc_context 中已把internal_buffer
// 各项清0，所以可以用base[0]来判断。
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic) {
	...
	// 校验视频图像的长宽是否合法。
	if (avcodec_check_dimensions(s, w, h)) return -1;
	// 如果没有分配内存，就分配动态内存并清0。
	if (s->internal_buffer == NULL)
		s->internal_buffer =
				av_mallocz(INTERNAL_BUFFER_SIZE * sizeof(InternalBuffer));
	// 取缓存中的第一个没有占用内存。
	...
	if (buf->base[0]) {
		...
		// 把长宽放大一些，比如在mpeg4
		// 视频中编码算法中的运动估计要把原始图像做扩展来满足不受限制运动矢量的要求(运动矢量可以超出原始图像边界)。
		w += EDGE_WIDTH * 2;
		h += EDGE_WIDTH * 2;
		...
		if (pixel_size == 3 * 8)
			...
			w = ALIGN(pixel_size * w, STRIDE_ALIGN << (h_chroma_shift + 3)) /
					pixel_size;
		...
		// 注意base[]和data[]数组还有作为标记的用途，free()时的非NULL
		// 判断，这里要清0。
...
// 释放占用的内存数组项。保证从0 到internal_buffer_count-1
// 数据项为有效数据，其他是空闲数据项
void avcodec_default_release_buffer(AVCodecContext *s, AVFrame *pic) {
	...
	for (i = 0; i < s->internal_buffer_count; i++) {
		...
		buf = &((InternalBuffer *)s->internal_buffer)[i];	// just 3-5 checks so is
																											 // not worth to optimize
		if (buf->data[0] == pic->data[0]) break;
	}
	...
	// 把将要空闲的数组项和数组最后一项交换，保证internal_buffer_count
	// 计算正确无误。 注意这里并没有内存释放的动作，便于下次复用已分配的内存
	temp = *buf;
	*buf = *last;
	*last = temp;
...
int avcodec_open(AVCodecContext *avctx, AVCodec *codec) {
	...
	if (avctx->codec) goto end;

	if (codec->priv_data_size > 0) {
		avctx->priv_data = av_mallocz(codec->priv_data_size);
		if (!avctx->priv_data) goto end;
	} else {
...
int avcodec_decode_video(AVCodecContext *avctx, AVFrame *picture,
	...
	*got_picture_ptr = 0;
	...
	if (buf_size) {
		...
		if (*got_picture_ptr) avctx->frame_number++;
	} else
		ret = 0;
...
int avcodec_close(AVCodecContext *avctx) {
	if (avctx->codec->close) avctx->codec->close(avctx);
...
AVCodec *avcodec_find_decoder(enum CodecID id) {
	...
	p = first_avcodec;
	while (p) {
		if (p->decode != NULL && p->id == id) return p;
		p = p->next;
	}
	return NULL;
}
