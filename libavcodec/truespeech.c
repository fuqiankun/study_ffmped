...
typedef struct TSContext {
	...
	int offset2[4];	// 7-bit value, encodes offsets for copying and for two-point
									 // filter
...
static void truespeech_correlate_filter(TSContext *dec) {
	...
	for (i = 0; i < 8; i++) dec->cvector[i] = (dec->cvector[i] * ts_230[i]) >> 15;
...
static void truespeech_filters_merge(TSContext *dec) {
	...
	if (!dec->flag) {
		...
		for (i = 0; i < 8; i++) {
			dec->filters[i + 0] =
					(dec->cvector[i] * 21846 + dec->prevfilt[i] * 10923 + 16384) >> 15;
			dec->filters[i + 8] =
					(dec->cvector[i] * 10923 + dec->prevfilt[i] * 21846 + 16384) >> 15;
		}
	}
	for (i = 0; i < 8; i++) {
...
static void truespeech_apply_twopoint_filter(TSContext *dec, int quart) {
	...
	for (i = 0; i < 146; i++) tmp[i] = dec->filtbuf[i];
...
static void truespeech_update_filters(TSContext *dec, int16_t *out, int quart) {
	...
	for (i = 0; i < 86; i++) dec->filtbuf[i] = dec->filtbuf[i + 60];
...
static void truespeech_synth(TSContext *dec, int16_t *out, int quart) {
	...
	for (i = 0; i < 60; i++) {
		int sum = 0;
		for (k = 0; k < 8; k++) sum += ptr0[k] * ptr1[k];
		sum = (sum + (out[i] << 12) + 0x800) >> 12;
		out[i] = clip(sum, -0x7FFE, 0x7FFE);
		for (k = 7; k > 0; k--) ptr0[k] = ptr0[k - 1];
		ptr0[0] = out[i];
	}

	for (i = 0; i < 8; i++) t[i] = (ts_5E2[i] * ptr1[i]) >> 15;

	ptr0 = dec->tmp2;
	for (i = 0; i < 60; i++) {
		int sum = 0;
		for (k = 0; k < 8; k++) sum += ptr0[k] * t[k];
		for (k = 7; k > 0; k--) ptr0[k] = ptr0[k - 1];
		ptr0[0] = out[i];
		out[i] = ((out[i] << 12) - sum) >> 12;
	}

	for (i = 0; i < 8; i++) t[i] = (ts_5F2[i] * ptr1[i]) >> 15;
...
static void truespeech_save_prevvec(TSContext *c) {
	...
	for (i = 0; i < 8; i++) c->prevfilt[i] = c->cvector[i];
}
