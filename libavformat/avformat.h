...
// 把音视频AVPacket组成一个小链表。
// 音视频数据包链表定义，注意每一个AVPacketList
// 仅含有一个AVPacket，和传统的很多很多节点的list不同，不要被list 名字迷惑。
...
static inline void av_free_packet(AVPacket *pkt) {
	if (pkt && pkt->destruct) pkt->destruct(pkt);
}
...
typedef struct AVIndexEntry {
	...
	int size : 30;	// yeah trying to keep the size of this small to reduce memory
									// requirements (its 24 vs 32 byte due to possible 8byte
									// align)
} AVIndexEntry;
...
typedef struct AVStream {
	...
	void
			*priv_data;	// AVIStream 关联解析各个具体媒体流与文件容器有关的独有的属性
...
// AVFormatParameters
// 结构在瘦身后的ffplay中没有实际意义，为保证函数接口不变，没有删除。
typedef struct AVFormatParameters {
	int dbg;	// only for debug
} AVFormatParameters;

// AVInputFormat 定义输入文件容器格式，着重于功能函数，
// 一种文件容器格式对应一个AVInputFormat结构，在程序运行时有多个实例，但瘦身后ffplay
// 仅一个实例。
typedef struct AVInputFormat {
	...
	struct AVInputFormat
			*next;	// 用于把ffplay 支持的所有文件容器格式链成一个链表。

} AVInputFormat;

// AVFormatContext
// 结构表示程序运行的当前文件容器格式使用的上下文，着重于所有文件容器共有的属性(并
...
typedef struct AVFormatContext	// format I/O context
{
	...
	void *
			priv_data;	// 指向具体的文件容器格式的上下文Context和priv_data_size配对使用
