				...
				if (stream_index >= s->nb_streams) {
					...
					switch (codec_type) {
						case CODEC_TYPE_VIDEO:	// BITMAPINFOHEADER
							...
							if (st->actx->extradata_size & 1) get_byte(pb);
						...
						case CODEC_TYPE_AUDIO: {
							...
							if (size > 16) {
								...
								actx->extradata_size =
										get_le16(pb);	// We're obviously dealing with WAVEFORMATEX
							...
							if (size % 2)	// 2-aligned (fix for Stargate SG-1 - 3x18 - Shades
														 // of Grey.avi)
								url_fskip(pb, 1);
...
int avi_read_packet(AVFormatContext *s, AVPacket *pkt) {
	...
	if (avi->non_interleaved) {
		...
		for (i = 0; i < s->nb_streams; i++) {
			...
			// 把帧偏移换算成帧数。
			if (ast->sample_size) ts /= ast->sample_size;
			// 把帧数换算成pts表示时间。
		...
		if (best_ast->remaining)
			i = av_index_search_timestamp(best_st, best_ts,
...
resync:
		...
		if (st->actx->codec_type == CODEC_TYPE_VIDEO) {
			if (st->index_entries) {
				...
				if (index >= 0 && e->timestamp == ast->frame_offset) {
					if (e->flags & AVINDEX_KEYFRAME) pkt->flags |= PKT_FLAG_KEY;
				}
			} else {
				pkt->flags |= PKT_FLAG_KEY;	// if no index, better to say that all
	...
	for (i = sync = url_ftell(pb); !url_feof(pb); i++) {
		...
		if (i >= avi->movi_end) break;

		for (j = 0; j < 7; j++) d[j] = d[j + 1];
		...
		if (i + size > avi->movi_end || d[0] < 0) continue;
...
static int avi_read_idx1(AVFormatContext *s, int size) {
	...
	if (nb_index_entries <= 0) return -1;
			 ...
			 i++)	// read the entries and sort them in each stream component
	{
		...
		if (i == 0 && pos > avi->movi_list) avi->movi_list = 0;
