	...
	if (ext) {
		...
		for (;;) {
			...
			*q = '\0';
			...
			if (!strcasecmp(ext1, ext)) return 1;
			if (*p == '\0') break;
			p++;
		}
	}
...
AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened) {
	...
	for (fmt1 = first_iformat; fmt1 != NULL; fmt1 = fmt1->next) {
		if (!is_opened) continue;

		score = 0;
		if (fmt1->read_probe) {
			// 读取文件头，判断文件头的内容来识别文件容器格式，这种识别方法非常可靠，设定score
			// 为100。
			score = fmt1->read_probe(pd);
		} else if (fmt1->extensions) {
			// 通过文件扩展名来识别文件容器格式，因为文件扩展名任何人都可以改，如果改变扩展名，这种方法就错误，如果不改变扩展名，这种识别方法有点可靠，综合等级为50。
			if (match_ext(pd->filename, fmt1->extensions)) score = 50;
		}
...
// 打开输入流，其中AVFormatParameters *ap 参数在瘦身后的ffplay
// 中没有用到，保留为了不改变接口
int av_open_input_stream(AVFormatContext **ic_ptr, ByteIOContext *pb,
	...
	//分配AVFormatContext
	//内存，部分成员变量在接下来的程序代码中赋值，部分成员变量在下面调用的ic->iformat->read_header(ic,
	// ap)函数中赋值。
...
int av_open_input_file(AVFormatContext **ic_ptr, const char *filename,
	...
	if (!fmt || must_open_file) {
		...
		file_opened = 1;
		// 如果程序指定ByteIOContext
		// 内部使用的缓存大小，就重新设置内部缓存大小。通常不指定大小。
		if (buf_size > 0) url_setbufsize(pb, buf_size);

		// 先读PROBE_BUF_MIN(2048)字节文件开始数据识别文件格式，
		// 如果不能识别文件格式，就把识别文件缓存以2
		// 倍的增长扩大再读文件开始数据识别，直到识别出文件格式或者超过131072
		// 字节缓存。
...
// 添加索引到索引表。有些媒体文件为便于seek，有音视频数据帧有索引，ffplay
// 把这些索引以时间排序放到一个数据中。返回值添加项的索引。
int av_add_index_entry(AVStream *st, int64_t pos, int64_t timestamp, int size,
											 int distance, int flags) {
	AVIndexEntry *entries, *ie;
	int index;

	if ((unsigned)st->nb_index_entries + 1 >=
			UINT_MAX / sizeof(AVIndexEntry))	// 越界判断
		return -1;

	entries =
			av_fast_realloc(st->index_entries, &st->index_entries_allocated_size,
	...
	} else	// 中插
	{
		...
		if (ie->timestamp != timestamp) {
			if (ie->timestamp <= timestamp) return -1;
...
AVStream *av_new_stream(AVFormatContext *s, int id) {
	...
	if (s->nb_streams >= MAX_STREAMS) return NULL;
